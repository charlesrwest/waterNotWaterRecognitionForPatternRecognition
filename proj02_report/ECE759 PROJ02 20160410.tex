...
\end{figure}
	...
	\subsection{SVM}
	Due to technical difficulties with the SVM code, we were unable to get visual output for this classification algorithm. We were able to attain error rates as low as .24. It is unclear for what value sof C and $\gamma$ this value was attained. Unfortunately, further testing showed that for a given set of parameter values for C and $\gama$, wide variations in the error were attained. In some cases, we were getting an error rate of greater than 50\%. We believe this is because of the way in which we passed the training data. We had to pass it 2500 pixels at a time. We decided to attempt to shrink the images to see if we could pass it a higher proportion of an image. However, we were unable to attain results from this fix.  \\
	\\
	While we attained an error rate using SVM that was less than either Bayes or CNN, we do not feel that our implementation of SVM was able to provide increased accuracy consistently. 
